...
local getGuidFromItemId = function(inventoryId, itemData, category, slotId)
    local outItem = Citizen.InvokeNative(0x886DFD3E185C8A89, inventoryId, itemData or 0, category, slotId)
...
local addWardrobeInventoryItem = function(itemName, slotHash)
    ...
    local itemData = Citizen.InvokeNative(0xCB5D11F9508A928D, inventoryId, nil, wardrobeItem, itemHash, slotHash, 1, addReason)

    if itemData then
        local equipped = Citizen.InvokeNative(0x734311E2852760D0, inventoryId, itemData, true)
        return equipped
    end
...
RegisterNetEvent('rsg-weapons:client:UseWeapon', function(weaponData, shootbool)
    ...
    local isThrowable = string.find(weaponName, 'thrown') ~= nil

    if not alreadyUsed and not UsedWeapons[hash] then
